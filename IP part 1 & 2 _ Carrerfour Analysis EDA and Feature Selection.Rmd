---
title: "IP part 1 & 2 _ Carrerfour Analysis EDA and Feature Selection"
author: "Ann Mberi"
date: "7/20/2021"
...
## We can also ggbiplot to plt our pca 

```{r}
library(ggbiplot)
```
# Adding more detail to the plot, we provide arguments rownames aslabels
#
```{r}
ggbiplot(ef.pca, labels=rownames(cf_df), obs.scale = 1, var.scale
= 1)
```
...
## Lets reload our data set calling it train 


```{r}
train <- read.csv(url("http://bit.ly/CarreFourDataset"))
```


# Loading our t_SNE library
#
...
#
```{r}
Quantity <- cf_df$Quantity
cf_df$Quantity <-as.factor(cf_df$Quantity)
```
...
#
```{r}
colors = rainbow(length(unique(cf_df$Quantity)))
names(colors) = unique(cf_df$Quantity)
```
...
text(tsne$Y, labels=cf_df$Quantity, col=colors[train$Quantity])
```
